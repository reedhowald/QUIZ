...
#         headerstring = txf + str(choice) + ' row ' + str(setline)
        ...
        print('256 header is:  ', headerstring, txf, choice)
...
#         font.setPointSize(int(str(runf[0][5])))
        ...
        if str(runf[6][12]) == '15':
            ...
            print('277 if is woeking', headerstring)
        print('278 runf[6][12] is ', runf[6][12], ccm, ccw, ccy)
        ry = str(runf[ir - 0][6])
        gy = str(runf[ir - 0][7])
        by = str(runf[ir - 0][8])
#         ry = '255'
...
# Subroutine to display a picture on the screen
    print('1571 displaying ir = ', ir, ' and ', choice, pic , '  ', filepathb)
# pic = 'C:/Users/Reed Howald/desktop/file1/footmenu1.png'
#     app = QA
#     pplication()
    w = QuizWindow()
...
def checkchoice(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, detf5, ctexcel, filepathb, ira, irb, fpk):
    print('checkchoice line 1592 with ', choice, ir )
...
# This is a shortcut for anyone who finds nx too cumbersome to type
        ...
        print('1615 choice of ', choice, ira, irb,ir)
#         switchba(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb)
        iraa =ira + 1
        irbb = irb+1

        print('1720 picture choices ', runf[iraa][3], setf5[iraa][3], runf[irbb][3], setf5[irbb][3] )
        iq = int(runf[4][8])
        if iq == 2:
            fic = 'rchcd.rng'
        ...
        print('1631 iq is ', iq, runf[4][8], pic)
        ir = ir - 1
        pic = runf[irbb][3]
        fp = filepath
        filepath = filepathb
...
#         pic = runf[1][3]
        pic = filepath + pic
        print('1639 pic is ', pic, ir , choice)
        dorunf(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb, fpk)
    elif choice == 'so':
        ir = 0
        dorunf(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb)
...
#         print('967 menuf[4][2] = ', menuf[4][2])
        if choice == str(menuf[iv][2]):
            print('1668 menuf[iv][2] = ', menuf[iv][2])
...
#                 setf5[0][iz] = value
                runf[2][11]  = value
                print('1678 the value is ', value, choice, iz, ir, iv)
            if iz > 8:
                iw = int(menuf[iv][5])
                iz = iz - 1
                print('1682 the value is ', value, choice, iz, ir, iv, iw)
...
def change(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb, fpk):
    print('!!  1766  in subroutine change() ', ir, ira, irb)
    print('1767 filepaths are ', fpk, filepath, filepathb, ira)
#     if ira == -1:
#         irq = 0#       ira = ira + 4
#         fpq = filepath
#     irq = irq +1
...
#         fcolor.write(str(colori))
        #         imax = setf5[1][3]
        print("1845 fpk is ", fpk, filepathb, colori)
        #         print('line 832 with imax = ', imax)
...
def ccount(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, detf5, ctexcel, filepathb, ira, irb, fpk):
    print('1898!!IN ccount WITH choice = ', choice)
    ccx = runf[5][8]
...
#     ccy = fr.readline()
    ...
    print('1914 ccn is ', choice, ccn, ccm)
    print('1915 ccw is  ', ccw, ccy)
#     choice = string(choice)
    tx1 = 'wrong' + ccc + ccn + '/n' + ccm
    tx1 = tx1 + 'correct' + czz + str(ccy)
    fr = open(filepath + 'tx1.txt' , 'w')
    fr.write(tx1)
    fr.close()
    fchoice = open(filepath+ 'choice.txt', 'r')
    choice = fchoice.read()
...
#             f2.write(str(ccy))
            ...
            czz = czz[0:-1]
            czz = czz + ' ' + choldb
            print('1951 ccy is now ', ccy, czz)

        return()
  # main()
# PROGRAM FOR CORRECT CHOICES
    print('1958 right choice is ', choice, ccx, cho )
    ...
    ccm = ccm[0:-1]
    ccm = ccm + ' ' + str(chold)
    print('1966 ccn is now ', ccn, ccm)

    ccn = int(ccn) + 1
    ccy = ccy[0:-1]
    ccy = ccy + ' ' + chold2
    print('1955 ccn is now ', ccn, ccm)
    print('1956 cchold is ', chold, choice, ccy)
    if czy == 2:
        return ()
    #         f2.write(str(ccw) + '\n')
...
#     s8 = fr.readline()
#     fr.close()
    print('2018 ccount.txt has been written with czz = ', czz)
...
print('2012 starting the program poly7.py')
print('2013 This is a polynomisl quiz ')
