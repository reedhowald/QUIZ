...
#         n 'C:/Users/Reed Howald/desktop/filew/'
        ...
        setfocus = runf[4][11]
        print('106', setline, setcolumn, setfocus)
        z = len(czy) - 1
# `       z = len(czy) - 1
        czy = czy[:z]
...
#         setting edit widget size
        ...
        # fsize = 0
        font = QFont()
        print('266 edit font size set to ', fsize, runf[0][5], setf5[0][5], setf5[5][11])
...
#         font.setPointSize(int(str(runf[0][5])))
        ...
        print('278 runf[5][8] is ', runf[5][8], ccm, ccw, ccy)
        ry = str(runf[ir - 0][6])
        gy = str(runf[ir - 0][7])
        by = str(runf[ir - 0][8])
#         ry = '255'
...
# the instructions for the edit widget use lines 200 through 259 at this time
        with open(filepath + "ivy.txt", 'r')as f2:
            ...
            ivz = f2.readline()
#             setline = f2.readline()
            print('318 ', ivy, ihx, ir)
        tfile = runf[ir][2]
        print('320 tfile = ', tfile, ir )
...
#             self.header.setText(headertext)
            ...
            QTimer.singleShot(delay1, self.close)
            print('666 !!! Start section  wjth chooice of ', choice, ir)
# self.close()
            return(value)
...
#             [['menu'], [3000], [2000], [0], [0], [0], [30], [1.0], [1.0], [1.0], [0.], [0.], [0.], [0.], [0.3], [0.1], [
#                 'menufoot.mnc']],
#   0], [2000], [0], [0], [0], [30], [1.0], [1.0], [1.0], [0.], [0.], [0.], [0.], [0.3], [0.1], [
#                 'menufoot.mnc']],
#             [['menu'], [3000], [2000], [0], [0], [0], [30], [1.0], [1.0], [1.0], [0.], [0.], [0.], [0.], [0.3], [0.1], [
#                 'menufoot.mnc']],
#             [['menu'], [3000], [2000], [0], [0], [0], [30], [1.0], [1.0], [1.0], [0.], [0.], [0.], [0.], [0.3], [0.1], [
...
#     print(str(runf.iloc[1][16]))
    ...
    print('1089 ir is ', ir, runf[ir-1][3])
...
# Subroutine to display a picture on the screen
    print('1570 displaying ir = ', ir, ' and ', choice , pic , '  ', filepathb)
# pic = 'C:/Users/Reed Howald/desktop/file1/footmenu1.png'
#     app = QA
#     pplication()
    w = QuizWindow()
    ...
    if z > 1:
        choice = choice[:z]
    print("1584 returned with ", choice, ir, z)
    print('in display() line 1585 with ', choice)
    fchoice.close()
#     close(app)
...
def checkchoice(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, detf5, ctexcel, filepathb, ira, irb, fpk):
    print('checkchoice line 1591 with ', choice, ir )
...
#         pic = runf[1][3]
        ...
        print('1644 qt IS TYPED!! closing the python program')
        quit()
    elif choice == 'rb':
        begin = 9
        ir = 0
...
#                 setf5[0][iz] = value
                runf[2][11]  = value
                print('1677 the value is ', value, choice, iz, ir, iv)
            if iz > 8:
                iw = int(menuf[iv][5])
                iz = iz - 1
...
def ccount(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, detf5, ctexcel, filepathb, ira, irb, fpk):
    print('1897 !!IN ccount WITH choice = ', choice)
    ccx = runf[5][8]
    print('1899 ccx  IS ', ccx, choice)
    if ccx != 12:
        return()
#  Program for incorrect choices
    fr = open(filepath +'ccount.txt', 'r')
    cho = fr.readline()
    ccc = fr.readline()
    ccn = fr.readline()
...
#e                return()
#         f2.write(str(ccw) + '\n')
        print('1934 WRONG choice choice is ', choice, ccc, ccn)
#             if czy == 2:
#                 return()
#     fc = open(filepath + 'chold.txt', r)
...
#         return()
        # main()
# PROGRAM FOR CORRECT CHOICES
        print('1958 right choice is ', choice, ccx, irb )
        print('RIGHT CHOICE WITH ', choice, ccc, runf[5][9])
        fr= open(filepath + 'fr.txt','r')
        s1   = fr.readline()
        s2 = fr.readline()
        s3 = fr.readline()
        s4 = fr.readline()
        ...
        print('1960 fr.txt has been read with s3 = ', s3)
    ...
    #     choice = string(choice)
        ...
        if z > 1:
            choice = choice[:z]
        ccz = runf[5][9]
        fc = open(filepath + 'choldc.txt', 'r')
        choldc = fc.read()
        if runf[5][9] != 15:
             ccz = int(ccz) + 1
             czy = czy[0:-1]
             czy = czy + ' ' + choldc
             print('1970 ccn is now ', ccn, ccm)
             print('1971 cchold is ', choldc, choice, ccy)
#         if czy == 2:
#             return ()
        #         f2.write(str(ccw) + '\n')
             print('1990 Right choice is ', choice, ccc, ccn)
        #     fc = open(filepath + 'chold.txt', r)
             with open(filepath + 'ccount.txt', 'w') as f2:
                f2.write(str(cho) + '')
                f2.write(str(ccc) + '')
...
#                 return ()
                ...
                print('2005 ccount.txt is printed  ccy js ',                                                                                                                                                                                                                                    ccy, ccw)
    # PROGRAM FOR CORRECT CHOICES
        print('2999 right choice is ', choice, ccy, irb)
        print('RIGHT CHOICE WITH ', choice, ccc, runf[5][9])

#         return()
# the following print statement will fe the first thing this python program does
print('2911 starting the program poly7.py')
print('1878 This is a polynomisl quiz ')
...
filepath=f.readline()
currentrun=f.readline()
print('1906 filepath is ', filepath + currentrun)
print('1907 currentrun is ', currentrun)
ir = 0
currentmenu=f.readline()
filepathb=f.readline()
runbi=f.readline()
...
print('1899 filepathb is ', filepathb, irb)
print('1900 starting with ', currentrun, get5f)
print(currentmenu)
