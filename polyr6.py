...
# filepath is read from controlstart.txt twice.  Here and at line 1694
        ...
        filepathi = 'C:/Users/Reed Howald/desktop/2file/'
#         filepathi = 'C:/Users/Reed Howald/Desktop/'
        ...
        print('86 filepathb is ', filepathb, filepath)
...
#         self.header.setStyleSheet("QLineEdit {background-color: rgb(" + rgba + ");}")
        print("287 top widget color  ", ry, gy, by)
        print('288 runf[0][2] is ', runf[0][2], setf5[0][2])
        fsize = 53
        fsize = runf[1][5]
...
#    line 183 is    self.input = QLineEdt()
        ...
        #         self.header.setStyleSheet("background-color): magenta," + settopfontsize)
        print('478 top font size is ', fsize, ir)
        self.user_keys.setFocus()
...
#     print(runf[2][16])
    ...
    print("1043 filename2 is ", 'ir is ', ir,filename2, getf5)
...
#             currentmenu = runf[ir][3]
            print(currentrun, ir, 'line 1231 ', currentrun)
            readfiles(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, getf5, ctexcel, filepathb, ira, irb, fpk, currentrun)
...
#             value = setf5[]
            ...
            irc = int(irc)
            ivz = int(ivz)
            print('1390 writing to setf5 ', setline, ir, iy, ivy, ihx)
            ir = ir + 1
            ivz = 2
            ijx = 0
            ivy = int(ivy)
            ivz = int(runf[3][4])
            ivz = int(ivz)
            print('1397 ivz = ', ivz, setline)
            while ijx < 17:
                setf5[setline - 1][ijx] = getf5[ivz][ijx]
                ijx = ijx + 1
            print("1401 ", setf5[setline - 1][3])
            setline = setline + 1
            ...
            with open(filepath + 'ivy.txt', 'w') as f2:
                ...
                f2.write(str(ivz) + '\n')
                f2.write(str(setline) + '\n')
            print('1513 finish writetosetf5() setline is ', setline, ivz)
            dorunf(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb)
            return ()
#         elif runf[ir][iy] == 'change':
...
def editf(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel):
    print('line 1443 in editf with ir = ', ir, choice)
    iq = ir
    ir = ir + 1
    d = timedelta()
#     d = datetime()
...
#     pplication()
    ...
    if z > 1:
        choice = choice[:z]
    print("1583 returned with ", choice, ir, z)
    print('in display() line 1584 with ', choice)
    fchoice.close()
#     close(app)
...
def checkchoice(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, detf5, ctexcel, filepathb, ira, irb, fpk):
    print('checkchoice line 1598 with ', choice, ir )
#     f.write(str(choice)+'\n')
#     f = open('c:/Users/Reed Howald/desktop/file1/outfile.txt','a+')
    f = open(filepath+'outfile.txt','a+')
    f.write(str(choice)+'\n')
...
#         switchba(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb)
        ...
        print('1585 iq is ', iq, runf[4][8], pic)
        ir = ir - 1
        pic = runf[irbb][3]
        fp = filepath
        filepath = filepathb
...
#         pic = runf[1][3]
        pic = filepath + pic
        print('1604 pic is ', pic, ir , choice)
        dorunf(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb, fpk)
    elif choice == 'so':
        ir = 0
        dorunf(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb)
        ...
        dorunf(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb)
    iv = 1
    print(menuf[1][4],' Line 1613 ', menuf[1][5])
    ir = ir + 1
    iw = 0
#     while iv < int(str(menuf[1][4])):
    print('1656 menuf[5][2] = ', menuf[5][2])
#     choice = str(choice)
...
#         print('967 menuf[4][2] = ', menuf[4][2])
        if choice == str(menuf[iv][2]):
            print('1664 menuf[iv][2] = ', menuf[iv][2])
...
#                 setf5[0][iz] = value
            ...
            if iz > 8:
                iw = int(menuf[iv][5])
                iz = iz - 1
                print('1678 the value is ', value, choice, iz, ir, iv, iw)
...
#             print('line 917 ', setf5[1][3])
            print('1689 iz  = ', iz, ir, iv, iw, value)
            runf[2][11] = value
            if iz == 6:
                change(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb, fpk)
#             setf5[1][iz] = value
#             runf[1][int(iz)] = setf5[1][iz]
            if iz < 8:
                print('1696 ', str(value), iz, runf[0][6])
#             runf[0][iz] = value
            elif iz > 7:
                runf[0][iz] = fvalue
            print('1700 New value at ', runf[0][iz], iz, ir, iv, iw, value, ira)
#             ir = ir+ 1
#             print('line 1009 selecting new ', currentrun)qt
#             ir = 0
            print("1704 iz = ", iz, ir, iv, iw, value)
#             switch = int(menuf[iw][6])

            switch = int(menuf[iv][6])
            print("1679 switch is ", switch, ira)
            fchoice = open(filepath + 'choice.txt', 'r')
            choice = fchoice.read()

            print('in checkchoice 1683 with:  ', choice, switch)
            if switch < 16:
...
#         iv = 120
#         dorunf(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f)
    print('1735 You typed ', choice, 'Please try again with a correct two letter combination')
#     f = open('c:/Users/Reed Howald/desktop/file1/outfile.txt','a+')
...
#     f.close()       replaced with better code
    with open(filepath + 'outfile.txt', 'a+')as f:
        f.write('BAD ' + str(choice) + ' not recognized here\n')
    ir = ir - 2
    print('1713 a bad choice has been made typing ', choice)
    pic = 'abc.png'
    pic = filepath + pic
    display(runf, menuf, ir, pic, choice, fchoice, filepath, setf5, f, runbi, ctexcel, filepathb, ira, irb, fpk)
...
print('1897 starting the program editrf6.py')
print('1878 This is menu driven programming in action ')
